...
def _compute_returns(
    ...
    try:
        ...
        stock_idx = stock_df.index.tz_localize(None) if stock_df.index.tz is not None else stock_df.index
        bench_idx = bench_df.index.tz_localize(None) if bench_df.index.tz is not None else bench_df.index
...
def main(
    ...
    fetch_end = (datetime.strptime(max_date, "%Y-%m-%d") + timedelta(days=holding_days + 14)).strftime("%Y-%m-%d")
    ...
    for _, row in df.iterrows():
        ...
        if stock_df is None or stock_df.empty:
            raws.append(None); alphas.append(None); holds.append(None)
            continue
        r, a, h = _compute_returns(stock_df, spy_df, row["analysis_date"], holding_days)
        raws.append(r); alphas.append(a); holds.append(h)
    ...
    if resolved.empty:
        console.print("[red]No rows have forward-return data yet. Wait a few trading days and re-run.[/red]")
        if export:
    ...
    if not buy.empty and not sell.empty:
        ...
        verdict = "[green]signal[/green]" if spread > 0.005 else (
            "[yellow]ambiguous[/yellow]" if spread > -0.005 else "[red]inverse[/red]"
        )
        console.print(
