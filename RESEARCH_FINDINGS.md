...
_**Updated 2026-05-07 (80+ PR day; bear-side mechanism class survey CONCLUDES)**: Spec 007 forward-catalyst gate (bull-active, bear-shadow) + Spec 008 Hybrid C calendar boost shipped 2026-05-06; **SC-009 live A/B ablation 2026-05-07: 36/36 rows COMPLETE; PRELIMINARY PASS-by-non-counterexample** — 0 decisions changed by boost across full sample; recommend SHADOW-MODE-FIRST for v2 default-on flip, NOT direct flip (PRs #57, #58, #61). Final canonical 21d-window verdict pending ~2026-05-22+. **Bear-side mechanism class survey from PR #22 CONCLUDES (6/6 evaluated)**: only **C-4 (institutional ownership delta)** cleared both Constitution VIII v1.4.0 standalone gate (n=12, mean α +5.41%, hit 75%, discrim +10.29pp; PR #75) AND v1.4.3 additive overlap vs Spec 007 (+8.06pp Δα improvement / +69pp hit improvement; C-4 catches 11 bearish commits Spec 007 entirely misses; PR #77). Spec X-1 (C-4 institutional rotation filter) is now spec-invocable; recommend SHADOW-MODE-FIRST. Two C-classes show INVERTED bear-side mechanism (C-2 short-covering + C-5 price-reaction): both originally hypothesized as mean-reversion; bear cohort empirically continuation. Three SKIP-types codified (empirical / data-availability / structural). Constitution v1.4.3 stable; drafts v1.4.4 (behavioral-additive 4th interpretation) + v1.4.5 (memory-log data-vs-prose discipline) shipped — drafts not yet ratified per defensive same-session rule. Path C analyst PT snapshot wiring (`tradingagents/agents/utils/analyst_pt_snapshot.py`) shipped default-OFF unlocking future C-3-class retrospectives. Spec 003 historical-recompute backfilled 254 cache rows; 9 tickers now clear FR-004 N≥20 floor (NVDA / AAPL / INTC / XLE / MSFT / GOOGL / JPM / XLF / XLK). Test count 1162 → **1179**. Cross-session memories 14 → **23**. Filter portfolio: 8 production + 1 spec-eligible candidate (C-4)._
...
## Filter portfolio status (added 2026-05-06; expanded 1 → 5 → 7 → **8 sides** as of late-evening)

The framework now ships with 7 distinct rating-suppression filter MODULES across **8 filter sides** in the PM hook chain (Spec 007 has independent bull + bear branches counted separately; Spec 008 is a hybrid layer inside Spec 007 bull). 5 sides default-on or default-shadow, 3 sides default-off. As of `v0.8.0-spec-008` tag (2026-05-06 late-evening): A3 has >30 supporting data points; spec 007 bull is empirically validated by Class 3 Opus retrospective DECISIVE PASS; spec 008 bull-only Hybrid C adds +3.35pp Δα beyond Class 3 alone at window=14d magnitude=0.5x but ships default-OFF pending live-mode ablation per SC-009; the rest are at small-sample or empirically rejected at their motivating defaults.
