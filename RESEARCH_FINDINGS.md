...
## Filter portfolio status (added 2026-05-06; expanded 1 → 5 → 7 → 8 → **9 sides + 1 conditional draft** as of 2026-05-09)

The framework now ships with 8 distinct rating-suppression filter MODULES across **9 filter sides** in the PM hook chain (Spec 007 has independent bull + bear branches counted separately; Spec 008 is a hybrid layer inside Spec 007 bull). 6 sides default-on or default-shadow, 3 sides default-off. **1 conditional spec drafted but not yet implemented** (Spec 012 Class 4 macro; PR #194 spec.md + plan.md). As of post-2026-05-09 state: A3 has >30 supporting data points; spec 007 bull is empirically validated by Class 3 Opus retrospective DECISIVE PASS; spec 008 bull-only Hybrid C adds +3.35pp Δα beyond Class 3 alone but ships default-OFF pending live-mode ablation per SC-009; Spec X-1 institutional rotation ships default-shadow at small-sample n=12; Spec 012 Class 4 macro retrospective PASSED at v1.4.0 + v1.4.3 gates (PR #193) but ships default-SHADOW per small-sample-caution (n=8 fires).
...
| **Spec 012 Class 4 macro BEAR** (CONDITIONAL DRAFT — `specs/012-class-4-macro-filter/`; PR #194 spec.md + plan.md only) | cross-asset/macro (VIX-snapshot threshold) | **(SHADOW planned)** | VIX < 18 (planned) | **+24.07pp net Δα / 75% cohort hit on n=8 retrospective fires** (PR #193) — PASS at v1.4.0 + v1.4.3 (mechanism-disjoint vs A3; +24.07pp incremental) | NOT YET IMPLEMENTED; spec.md + plan.md scaffolded; MVP/tests/polish deferred. First macro-environment-aware filter. |
