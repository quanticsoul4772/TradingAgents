...
|---|---|---|---|
| **L4 (pilot v1+v2 landed 2026-05-09; status NARROWED to fundamentals-stage focus)** | **battlecode2026 ratbot6** | **Squeak (structured signaling) — BR-3 v1+v2** | Analysts emit `{bullish, bearish, key_risks}` instead of prose. **BR-3 v1 (market analyst, $8, n=20, PR #178): PARTIAL ALT-B** — commit shift +20pp triggered but α delta +0.24pp below threshold. **BR-3 v2 (news + fundamentals analysts, $16, n=40, PR #225): DIFFERENTIAL** — sub-A `news_analyst` NULL-leaning (commit shift 0pp consistent across both NVDA + AAPL); sub-B `fundamentals_analyst` PARTIAL ALT-B (commit shift **+40pp** — 2× the v1 magnitude; consistent across both tickers). **2 of 3 analyst stages carry the structured-output bottleneck** (market + fundamentals); 1 of 3 does NOT (news). Asymmetric mechanism: tools-rich analysts carry it; narrative analyst doesn't. Phase E STILL NOT unblocked at this evidence level (calibration unproven; α magnitudes BELOW ±1pp). L4 status NARROWED from "pilot-eligible" to **"pilot-eligible (focus on fundamentals analyst stage)"** — fundamentals carries the strongest commit-shift signal (+40pp). Next-step extension: BR-3 v3 (combined market + fundamentals structured) at ~$8-16; tracked as Open Question post-2026-05-09. |
...
| ~~Does a Class 4 (cross-asset/macro) filter catch the bear-side ticker_strong cohort?~~ — **RESOLVED 2026-05-09**, verdict **PASS at both v1.4.0 standalone AND v1.4.3 additive gates** per `claudedocs/class4-macro-filter-retrospective-2026-05-09.md`. Recommended default: VIX < 18 snapshot threshold; n=8 fires; net Δα +24.07pp; cohort hit 75% (6 of 22 ticker_strong cohort caught). v1.4.3 additive vs A3: +24.07pp incremental (mechanism-disjoint — A3 catches 0 of 22 ticker_strong by definition; Class 4 catches 6). Recommendation: PROCEED to spec drafting at default-SHADOW mode (small-sample-caution per v1.4.0 sub-clause); 30+ live fires before default-on flip. Discriminator: VIX 30d Δ% (+10.50% ticker_strong vs +22.96% other-bear-cells; -12.46pp Δ). | (resolved) | $0 retrospective; $0 future spec |
| ~~Does a Class 5 (fundamentals-delta) filter add discrimination beyond Class 3?~~ — **RESOLVED 2026-05-06**, verdict **SKIP** per `claudedocs/forward-catalyst-class5-vs-class3-overlap-2026-05-06.md`. Standalone gate PASSED at T=0.02 (discrim +11.92pp / hit 96.3% / net Δα +4.37pp on n=47), but additive-to-existing-filter analysis showed 31 of 41 Class 5 fires (76%) overlap with Spec 007 (Class 3) — and the union of {Class 3 fires} ∪ {Class 5 fires} hurt net Δα by -4.09pp vs Spec 007 alone (Class 5's incremental fires were 8 false-positive winners + 2 incremental losers). Constitution VIII v1.4.3 (additive-to-existing-filter gate) was created in direct response to this finding to prevent shipping a redundant filter. | (resolved) | $0 |
...
| ~~Does a BULL-side Class 4 macro filter catch the 27-row ticker_weak cohort (5th-failure-mode)?~~ | Class 4 macro BULL retrospective (PR #203). | $0 | **RESOLVED 2026-05-09 — SKIP** at every VIX threshold tested (16-30). Best threshold (VIX > 16): n=67 fires, net Δα -2.25pp, cohort hit 30%. Asymmetric to bear-side: ticker_weak cohort committed in LOWER VIX environments (20.64 vs 22.65) — opposite of symmetric hypothesis. Mechanism is stock-specific mean-reversion (88.9% Tech-concentrated; AAPL/MSFT/NVDA local highs), NOT macro environment failure. Spec 012 `class_4_macro_bull_mode = "off"` default empirically correct. |
| ~~Can per-ticker-local-high (price-vs-30d-rolling-MAX) catch the bull ticker_weak cohort identified by Class 4 BULL SKIP analysis?~~ | Local-high BULL retrospective (PR #205). | $0 | **RESOLVED 2026-05-09 — DEFER (POTENTIAL PASS at sample-size floor)**. Best threshold (price within 1% of 30d high): n=2 fires, net Δα +6.39pp, cohort hit 100% — TECHNICAL PASS but n=2 < Spec 012 n=8 / Spec X-1 n=12 floors. At broader thresholds (-2% to -8%), false-positive rate dominates and net Δα turns negative. Future re-evaluation triggers: bull-cohort growth to 150+ commits OR alternative mechanisms (60d high; distance-to-analyst-PT; proximity AND bull_keyword_count combined). |
| ~~Does WC-11 v2 disambiguation (3 tickers × 5 dates × 4 perms = 60 propagates) resolve ALT-A vs ALT-B vs NULL/PARTIAL/INCONCLUSIVE on the analyst-order question?~~ | WC-11 v2 (PR #228 ANALYSIS). | $24 (T2; user-authorized) | **RESOLVED 2026-05-09 evening — PARTIAL** + ticker-asymmetry. NVDA reproduces v1 news_fundamentals_market elevation EXACTLY (40% in both); AAPL elevates with news-LAST orderings (60% on fundamentals_market_news + market_fundamentals_news; OPPOSITE of NVDA); MSFT shows fundamentals-early pattern. **No single analyst-position rule explains all 3 tickers**. v1 ALT-A vs ALT-B ambiguity JOINED by ticker-asymmetry rather than disambiguated. Constitution Patch D from PR #215 conditional drafts applied as v1.5.3 (CLARIFY ticker-conditional). |
| ~~Does BR-3 v2 (news + fundamentals analyst structured-output) generalize the BR-3 v1 +20pp commit-shift effect beyond market analyst?~~ | BR-3 v2 (PR #225 ANALYSIS). | $16 (T2; user-authorized) | **RESOLVED 2026-05-09 evening — DIFFERENTIAL**: sub-A news_analyst NULL-leaning (commit shift 0pp); sub-B fundamentals_analyst PARTIAL ALT-B (commit shift **+40pp**; α delta +0.11pp below ±1pp threshold). 2 of 3 analyst stages carry the bottleneck (market + fundamentals); 1 of 3 does NOT (news). Asymmetric mechanism: tools-rich analysts carry the structured-output bottleneck; narrative analyst doesn't. Phase E NOT unblocked at this evidence level (calibration unproven; α magnitudes BELOW ±1pp). Cross-pollination L4 status updated to "pilot-eligible (focus on fundamentals analyst stage)". |
| Can the behavioral-additive escape clause (Constitution v1.4.6 + Spec 011) ever be invoked, or is bear-side mechanism survey COMPLETE making it dormant? | Future state-log accumulation may reveal new bull-side filter candidates that fail v1.4.3 actual-fires gate but pass v1.4.6 counterfactual gate. Bear-side survey is now **OPEN-ENDED at 7-evaluated / 2 PASS** post-2026-05-09 (Class 4 macro = Spec 012 joined C-4 institutional rotation as second spec-eligible bear-side mechanism class; PASS at v1.4.0 standalone + v1.4.3 additive vs A3). The "closed" framing was stale — Class 4 itself was NEW post-2026-05-07 survey-conclusion, demonstrating new mechanism class hypotheses can surface. | $0 (passive — depends on accumulated state logs) | **deferred (Spec 011 first invocation); survey re-opened post-Class-4** |
| Does the WC-11 v2 ticker-conditional analyst-order pattern (NVDA news-first vs AAPL news-last vs MSFT fundamentals-early) generalize across a broader ticker basket, or is it sensitive to the specific 3-ticker mix? | WC-11 v3-class extension (broader ticker basket; ≥5 tickers × 5 dates × 4 perms). | ~$40 (T3) | **open (NEW post-WC-11-v2-PARTIAL 2026-05-09)** |
| Does combining market + fundamentals structured-output (the 2 stages BR-3 v2 confirmed carry the bottleneck) push α delta above the ±1pp ALT-B calibration threshold and unblock Phase E? | BR-3 v3-class extension (combined market + fundamentals structured modes). | ~$8-16 (T2) | **open (NEW post-BR-3-v2-DIFFERENTIAL 2026-05-09)** |
