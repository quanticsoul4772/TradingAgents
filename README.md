...
After 24 experiments + cross-experiment horizon sweep + per-ticker breakdown + Opus 4.7 model swap (NVDA + AAPL) + Opus 30-pair mixed basket (Q1 2026) + 3-period NVDA cross-validation (Q1 2026 + Q4 2025 + Q3 2025) + Phase D substrate exploration (XLK + multi-sector + XLE) + SC-003 50-ticker validation + A3 filter forensics + Spec 002 signal-lifecycle pipeline + Spec 001 bots-architecture (Phases 1-5) + Spec 003 analyst-stage contrarian gate (Phases 1+2 implemented + SC-001/SC-002/SC-003 validated; default-on flipped 2026-05-06) + Spec 003.5 sector-baseline fallback + Spec 004 sector-momentum filter + Spec 006 bear-sector-symmetry filter + Spec 007 forward-catalyst gate (bull-active, bear-shadow) + Spec 008 Hybrid C calendar boost + sector-α attribution analyzer + Constitution v1.4.3 → v1.4.5 → v1.4.6 (Quality Gate #6 Memory-log discipline + Behavioral-additive 4th interpretation, both ratified 2026-05-07) + **Spec 008 SC-009 live A/B ablation (2026-05-07; 36/36 rows COMPLETE; PRELIMINARY PASS-by-non-counterexample — 0 decisions changed by boost across full sample; recommend SHADOW-MODE-FIRST for v2 default-on flip)** + **bear-side mechanism class survey from PR #22 CONCLUDES (6/6 evaluated; C-4 institutional ownership delta is SOLE spec-eligible — passed standalone gate @ n=12 + v1.4.3 additive overlap @ +8.06pp Δα improvement vs Spec 007)** + **Spec X-1 (C-4 institutional rotation filter) DEPLOYED end-to-end via 6-PR bundle (#88 spec → #89 plan → #90 tasks → #91 MVP → #92 tests → #93 polish; default-shadow bear-side, default-off bull-side; 18 tests + 4 config keys + ~190 LOC helper module)** + **Spec 010 (Hybrid D bear-side calendar-boosted) closed via SKIP retrospective (PR #86; 3-converging-retrospective methodological closure of bear-side calendar-boost mechanism class)**:
...
**Bear-side mechanism class survey CONCLUDES 2026-05-07**: 6/6 candidate mechanism classes from PR #22 design doc evaluated. **C-4 (institutional ownership delta) is the SOLE spec-eligible mechanism class** — passed Constitution VIII v1.4.0 standalone gate (n=12, mean α +5.41% on suppressed bearish, hit 75%, discrim +10.29pp) AND v1.4.3 additive overlap vs Spec 007 (+8.06pp Δα improvement / +69pp hit improvement; C-4 catches 11 bearish commits Spec 007 entirely misses — different signal sources: LLM semantic vs quantitative 13F flow). **C-4 is now DEPLOYED as Spec X-1** via 6-PR bundle (#88-#93; default-shadow bear-side, default-off bull-side; 18 tests + 4 config keys + ~190 LOC helper module). Two C-classes show INVERTED bear-side mechanism (C-2 short-covering + C-5 price-reaction): both originally hypothesized as mean-reversion; bear cohort empirically continuation. Three SKIP-types codified: empirical (C-1, C-2, C-5 reaction), data-availability (C-3), structural (C-6). **Spec 010 (Hybrid D bear-side calendar-boosted forward-catalyst filter) closed via SKIP retrospective (PR #86)**: structural argument shows the calendar-boost mechanism cannot operate on the n=5 behavioral-additive bear cohort (4-of-5 cases have pre_rating=Hold so nothing to suppress); 3-converging-retrospective methodological closure of the bear-side calendar-boost mechanism class. **Constitution v1.4.5 (Quality Gate #6 — Memory-log data-vs-prose discipline) + v1.4.6 (Behavioral-additive 4th interpretation; PM-as-multi-mechanism-validator reframe) both ratified 2026-05-07** (PRs #83 + #84; v1.4.4 draft content ratified as v1.4.6 to preserve monotone numbering after v1.4.5 ratified first per reasoning_decision rank ordering).
...
## Filter portfolio (9 production filters as of 2026-05-07)
...
| **[Spec X-1 C-4 institutional rotation](specs/091-c4-institutional-rotation/)** — DEPLOYED 2026-05-07 | quantitative 13F flow (NEW class) | SHADOW bear-side @ T_outflow=-5% / OFF bull-side | Standalone PASS @ n=12 (PR #75) + v1.4.3 ADDITIVE PASS @ +8.06pp Δα (PR #77); bear-side; SOLE survivor of 6/6 PR #22 mechanism class survey; 6-PR bundle (#88-#93) shipped end-to-end |

5 of 9 production filters default-active or shadow (A3 active, spec 003 active, spec 003.5 active, spec 007 bull active, spec 007 bear shadow); spec X-1 default-shadow bear-side. 8 candidates retrospectively SKIPPED before any spec written or via post-implementation closure: **Spec 005 candidate** (per-ticker-vs-sector BULL, max +0.31pp), **Spec 009 candidate** (bear-inverted Hybrid C, +0.00pp at every config), **5 of 6 bear-side mechanism classes** from PR #22 (Class C-1 SKIP empirical / C-2 SKIP mechanism-inverted / C-3 SKIP not-feasible / C-5 SKIP both variants / C-6 SKIP structural), and **Spec 010 (Hybrid D bear-side calendar-boosted)** closed via PR #86 SKIP retrospective on structural-incompatibility argument. Pre-spec validation per Constitution Principle VIII saved ~30-40h of empty-spec implementation across the 6-class survey. See [`claudedocs/research-burst-2026-05-06.md`](claudedocs/research-burst-2026-05-06.md) and [`claudedocs/research-burst-2026-05-07.md`](claudedocs/research-burst-2026-05-07.md) for meta-retrospectives.
...
- `.specify/memory/constitution.md` — **eight** principles governing research approach (v1.4.6; Principle VIII added 2026-05-06 with v1.3.0 retrospective-first base, then extended same-day v1.4.0 forward-catalyst-class gate / v1.4.1 spec ships its retrospective / v1.4.2 magnitude fungibility for hybrid filters / v1.4.3 additive-to-existing-filter gate; v1.4.5 added Quality Gate #6 Memory-log data-vs-prose discipline 2026-05-07; v1.4.6 appended Behavioral-additive 4th interpretation sub-section to Principle VIII v1.4.3 2026-05-07. Original v1.4.4 draft content was ratified as v1.4.6 to preserve monotone numbering after v1.4.5 was ratified first per reasoning_decision rank ordering).
...
- `tradingagents/agents/utils/institutional_rotation_filter.py` — Spec X-1 C-4 institutional rotation filter (sums top 10 institutional holders' pctChange via yfinance, LRU-cached). Bear-side **default-shadow** at T_outflow=0.05 per Constitution VIII v1.4.0 small-sample caution (n=12 cohort). Bull-side **default-off** (n=1 evidence too thin). Set `institutional_rotation_bear_mode = "active"` to flip after shadow-mode validation; set both modes to `"off"` for zero overhead. Standalone PASS at n=12 + v1.4.3 ADDITIVE PASS at +8.06pp Δα improvement vs Spec 007. SC-009 deferred re-validation gate: re-run `scripts/forward_catalyst_class4_retrospective.py` after Q1 2026 13F refresh ~2026-05-15.
...
**Eight** principles in [`.specify/memory/constitution.md`](.specify/memory/constitution.md) (v1.4.6): Save Everything, One Experiment Per Change, Stay Cheap (4-tier ladder T1 ≤$5 / T2 $5-30 / T3 $30-100 / T4 >$100), No Production Claims, Steal Liberally, Spec Before Structural Change (extended v1.4.1 with "Spec ships its retrospective + verdict"), **Calibrated Abstention is a Valid Output** (with 2026-05-03 Replicability-scope + Cross-period-scope clarifications), **Retrospective Before Spec for Backward-Looking Price Filters** (added v1.3.0 2026-05-06; extended same-day with v1.4.0 forward-catalyst-class gate / v1.4.2 magnitude fungibility for hybrid filters / v1.4.3 additive-to-existing-filter gate; extended 2026-05-07 with v1.4.6 Behavioral-additive 4th interpretation sub-section). Quality Gate #6 added 2026-05-07 (v1.4.5): Memory-log data-vs-prose discipline — operators MUST cross-check entry header data against reflection prose before citing prior memory log entries as evidence. The principles are constraints, not aspirations.
