...
**Baseline** (post-2026-05-05 tech-debt sweep — see `claudedocs/tech-debt-2026-05-05.md`): ruff = **0 errors** (down from 305 at scaffolding install — see commit `a0ec447`); mypy = 175 errors. The mypy floor is dominated by structural config-typing churn (`DEFAULT_CONFIG` returning `Any | dict | float | int | str | None` floods `conditional_logic.py` and the LLM clients with arg-type/union-attr noise); fixing it cleanly needs a TypedDict for `DEFAULT_CONFIG` — its own project, deferred. Net-new code must keep ruff at zero.
