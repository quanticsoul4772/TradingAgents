...
- **Spec 012 Class 4 macro-environment filter** (`specs/012-class-4-macro-filter/`, 2026-05-09; CONDITIONAL DRAFT — spec.md + plan.md only via PR #194; MVP/tests/polish deferred) — proposed module `tradingagents/agents/utils/macro_environment_filter.py`. **FIRST macro-environment-aware filter** in the framework. Suppresses Underweight/Sell commits to Hold when the VIX snapshot is below threshold (default 18.0; recommended per the 2026-05-09 retrospective `claudedocs/class4-macro-filter-retrospective-2026-05-09.md`). Bear-side **default-SHADOW (planned)** at T=18.0 per Constitution VIII v1.4.0 small-sample-caution sub-clause (n=8 fires from retrospective: discrim +24.07pp / hit 75% / net Δα +24.07pp). Bull-side **default-OFF (planned)**. Filter ordering (FR-004): A3 → Class 4 → Spec X-1 (smallest-sample-last). 3 new `TradingAgentsConfig` keys (planned): `class_4_macro_bear_mode` (Literal[off/shadow/active], default shadow), `class_4_macro_bull_mode` (default off), `class_4_macro_vix_threshold` (default 18.0). State annotation `state["class_4_macro"]` with 7 fields (`vix_snapshot`, `vix_30d_pct`, `tnx_30d_pct`, `dxy_30d_pct`, `would_fire_bear`, `fired_bear`, `pre_rating`, `post_rating`). Zero LLM cost (Constitution III T0); ~250ms p99 latency cache-cold. **Constitution VIII v1.4.0 + v1.4.3 gates pre-cleared by PR #193 retrospective**: standalone PASS at n=8, additive PASS vs A3 (mechanism-disjoint — A3 catches 0 of 22 ticker_strong cohort by definition; Class 4 catches 6). Mechanism class distinction: distinct from per-ticker price (A3) / per-sector (Spec 004/006) / prose-density (Spec 003) / LLM-extracted (Spec 007/008) / institutional flow (Spec X-1). Naming clarification: "Spec 012" / "Class 4" = Spec 008 design doc mechanism classification; NOT to be confused with C-4 institutional rotation = Spec X-1 (bear-side mechanism survey numbering). Default-on flip per SC-010 deferred until 30+ live shadow-mode fires accumulate.
