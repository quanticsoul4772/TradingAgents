...
After 33 experiments + cross-experiment horizon sweep + per-ticker breakdown + Opus 4.7 model swap + Opus 30-pair mixed basket + 3-period NVDA cross-validation (Q1 2026 / Q4 2025 / Q3 2025) + Phase D substrate exploration + Phase C reasoning_evidence wiring + Spec 002 signal-lifecycle + Spec 001 bots-architecture (Phases 1-5) + Spec 003 contrarian gate (default-on @80%) + Spec 003.5 sector-baseline fallback + Spec 007 forward-catalyst gate (bull-active, bear-shadow) + Spec 008 Hybrid C calendar boost + Spec X-1 institutional rotation filter (default-shadow bear) + **Spec 012 Class 4 macro-environment filter (default-shadow bear @ VIX < 18; FIRST cross-asset/macro filter; deployed end-to-end via 5-PR bundle 2026-05-09 PRs #194-#200)** + **SC-003 50-ticker signal-at-scale validation** + **Spec 008 SC-009 PRELIMINARY PASS-by-non-counterexample** + **bear-side mechanism class survey at 7-evaluated / 2 PASS (C-4 + Class 4 spec-eligible; survey OPEN-ENDED — Class 4 was NEW post-2026-05-07 conclusion, demonstrating new mechanism class hypotheses can surface)** + **WC-10 research arc CLOSED 2026-05-09**: v1 (n=20, SC-007 ALT-A confirmed at 3.6× commit ratio) + v2 (n=80 across 8 tickers, **SC-005(b) NULL** at combined v1+v2 Pearson r +0.0918 / Spearman ρ +0.0410 below ±0.197 critical at p=0.05; SC-007 ALT-A PARTIAL on 5/8 tickers; JNJ + GOOG + JPM retain Hold-default → VII original sub-population; bullish-amplification REPLICATES with Buy n=20 α +2.93% / 80% hit) + v3 (Q4 2025 NVDA PARTIAL ALT-A within ±100bps NULL) + **Spec 009 Branch C ACTIVATED** (bin-then-output ergonomic-only mode; new PARAMS key `wc_10_internal_only=True`; 5-tier external preserved; continuous internal for audit; `daily_signals.py` does NOT expose WC-10 flags) + **WC-11 analyst-order randomization PARTIAL ALT-A + ALT-B** (n=20; per-permutation commit rate 0% → 40%; cannot disambiguate first-speaker vs last-speaker bias at this n; Constitution VII v1.5.1 → v1.5.2 "Analyst-order scope" paragraph) + **WC-11 v2 ticker-conditional disambiguation PARTIAL** (n=60 across NVDA + AAPL + MSFT × 5 dates × 4 perms; NVDA reproduces v1 EXACTLY at `news_fundamentals_market` 40%; AAPL elevates with NEWS-LAST orderings 60%; MSFT elevates fundamentals-EARLY; no single analyst-position rule explains all 3 tickers; Constitution VII v1.5.2 → v1.5.3 "Ticker-conditional clarification" paragraph) + **BR-3 Squeak market-analyst structured-output PARTIAL ALT-B** (n=20; commit shift +20pp triggered but α delta +0.24pp below ALT-B magnitude threshold; Phase E NOT unblocked) + **BR-3 v2 analyst-stage DIFFERENTIAL** (n=40 across NVDA + AAPL × 5 dates × 4 modes; sub-A `news_analyst` NULL-leaning 0pp commit shift; sub-B `fundamentals_analyst` PARTIAL ALT-B +40pp commit shift; 2 of 3 analyst stages carry the structured-output bottleneck — market + fundamentals; news does NOT; Phase E STILL not unblocked at this evidence level) + **Spec 011 behavioral-additive operational procedure** (codifies Constitution v1.4.6 invocation pattern) + **Class 4 BULL SKIP + Class 5 BULL re-run SKIP confirmed + local-high BULL DEFER (4 retrospectives validated Constitution VIII v1.4.1 retrospective-first methodology in 1 day; 2026-05-09 PRs #202 + #203 + #205)**, the architectural conclusion is captured in **`RESEARCH_FINDINGS.md`**. Forward roadmap in **`ROADMAP.md`**.
...
This is encoded in **Constitution Principle VII (Calibrated Abstention is a Valid Output)** added 2026-05-03 then re-amended through 2026-05-09. **v1.5.0 added "Schema-induced abstention is NOT calibrated abstention" sub-section** after WC-10 v1 (NVDA case: 8 of 10 5-tier Holds were schema-suppressed bullish reads that continuous-scalar mode emitted as Buy/OW with realized 21d α from +2.83% to +8.53%). **v1.5.1 added "Bear-regime validation" paragraph** after WC-10 v3 PARTIAL ALT-A verdict on Q4 2025 NVDA cohort (α delta -0.22pp within ±100bps NULL region; magnitude < 1.0pp). **v1.5.2 added "Analyst-order scope" paragraph** to the Replicability sub-section after WC-11 ALT-A + ALT-B verdict (per-permutation commit rate 0% → 40%; DEFAULT [market, news, fundamentals] order is empirically Hold-biased). **v1.5.3 added "Ticker-conditional clarification" paragraph** to the Analyst-order scope sub-section after WC-11 v2 PARTIAL verdict (NVDA news-first elevation REPLICATES at 40%; AAPL elevates news-LAST orderings 60%; MSFT elevates fundamentals-EARLY; no single analyst-position rule explains all 3 tickers — analyst-order effect is TICKER-CONDITIONAL, not framework-general first-speaker bias). VII still applies to commits whose evidence is GENUINELY BALANCED (the original framing). The v1.5.0 carve-out covers cases where evidence is one-directional but moderate-magnitude AND the schema lacks a partial-confidence tier. v1.5.1 documents the empirical magnitude bound and notes that runtime monitoring via `scripts/wc_10_underperformance_monitor.py` (PR #146) suffices in lieu of regime-aware gating. v1.5.2 mandates that future commit-rate ablations randomize analyst order or document as confounder. v1.5.3 mandates that ticker-asymmetry be explicitly tested at any commit-rate ablation across multiple tickers. Any new structural change that reduces Hold rate must justify in HYPOTHESIS.md which sub-population it targets and document the directional asymmetry expectation.

## Read the constitution first

The design is governed by `.specify/memory/constitution.md`. Before non-trivial changes, read:

1. `.specify/memory/constitution.md` — **eight** core principles (Save Everything, One Experiment Per Change, Stay Cheap, No Production Claims, Steal Liberally, Spec Before Structural Change, **Calibrated Abstention is a Valid Output**, **Retrospective Before Spec for Backward-Looking Price Filters**). **Current version: v1.5.3** (2026-05-09, PATCH — added "Ticker-conditional clarification" paragraph to Principle VII Analyst-order scope sub-section per WC-11 v2 PARTIAL verdict). Recent amendment history: v1.3.0 → v1.4.0 (forward-catalyst-class gate) → v1.4.1 (spec ships its retrospective) → v1.4.2 (magnitude fungibility) → v1.4.3 (additive-to-existing-filter gate) → v1.4.5 (Quality Gate #6 memory-log discipline) → v1.4.6 (behavioral-additive 4th interpretation; v1.4.4 draft ratified as v1.4.6 to preserve monotone numbering) → v1.5.0 (schema-induced abstention carve-out per WC-10 v1) → v1.5.1 (bear-regime magnitude bound per WC-10 v3) → v1.5.2 (analyst-order scope per WC-11) → **v1.5.3 (ticker-conditional clarification per WC-11 v2)**. The principles are constraints, not aspirations.
...
**Test coverage** (as of 2026-05-09 post-dual-pilot landing + Spec 012 deployment): **1193 tests passing** (was 852 → 984 → 1022 → 1146 → 1193 across 2026-05-06/07/08/09; +47 since 2026-05-08; +341 net since 2026-05-04 baseline), 81%+ project coverage. Anthropic + OpenAI clients ~95%, all 4 analyst factories + 5 debate-agent factories 100%, graph/setup.py 100%, conditional_logic.py 100%, trading_graph.py 88%, dataflows/interface.py 98%, memory.py 94% (per PR #116 corrected measurement; the 9.7% figure that appeared in 3 earlier coverage surveys was a `pytest-cov` collection-error artifact). New since 2026-05-04: Spec 003 contrarian gate (30 unit tests); Spec 002 within-ticker IC integration (8 tests); analyzer counterfactual-integration contract (2 tests); WC-10 (10 unit tests for `bin_scalar_to_tier` + 2 PM integration tests for filter-bypass mode). Spec 002 signal-lifecycle (Phases 0-2.5) at ~95%, Spec 001 bots-architecture (Phases 1-5) including Phase 4 BotLLMFactory live-validated, Spec 003 contrarian gate (Phases 1+2) live-validated end-to-end (SC-001 + SC-002), WC-10 v1 live-validated (`experiments/2026-05-08-001-wc-10-pilot/`). Routing-mismatch regression test (`tests/test_interface_routing.py::test_every_categorized_tool_has_impl_for_default_vendor`) added 2026-05-03 after experiment 008 caught the `get_insider_transactions × news_data × exa` mismatch. The 2026-05-08 mypy sweep (PRs #117-#129) added zero tests — typing-only changes; the existing 1146-test suite verified zero regressions across each PR.
...
- **Spec 012 Class 4 macro-environment filter** (`specs/012-class-4-macro-filter/`, 2026-05-09; **DEPLOYED end-to-end via 5-PR bundle (#194 + #197 + #198 + #199 + #200)** at default-SHADOW) — module `tradingagents/agents/utils/macro_environment_filter.py`. **FIRST macro-environment-aware filter** in the framework. Suppresses Underweight/Sell commits to Hold when the VIX snapshot is below threshold (default 18.0; recommended per the 2026-05-09 retrospective `claudedocs/class4-macro-filter-retrospective-2026-05-09.md`). Bear-side **default-SHADOW** at T=18.0 per Constitution VIII v1.4.0 small-sample-caution sub-clause (n=8 fires from retrospective: discrim +24.07pp / hit 75% / net Δα +24.07pp). Bull-side **default-OFF**. Filter ordering: A3 → Spec 003/003.5 → Spec 004 → Spec 006 → Spec 007 → Spec X-1 → **Class 4 (LAST)** per smallest-sample-last rule (n=8 cohort < Spec X-1 n=12). 3 new `TradingAgentsConfig` keys: `class_4_macro_bear_mode` (Literal[off/shadow/active], default shadow), `class_4_macro_bull_mode` (default off), `class_4_macro_vix_threshold` (default 18.0). State annotation `state["class_4_macro"]` with 7 fields (`vix_snapshot`, `vix_threshold`, `bear_mode`, `would_fire_bear`, `fired_bear`, `pre_rating`, `post_rating`); when bear_mode is "off", the dict is NOT added (FR-011 backward-compat). Persisted via `_log_state` whitelist. AgentState TypedDict extended. Zero LLM cost (Constitution III T0); ~250ms p99 latency cache-cold. **Constitution VIII v1.4.0 + v1.4.3 gates PASSED in PR #193 retrospective**: standalone PASS at n=8, additive PASS vs A3 (mechanism-disjoint — A3 catches 0 of 22 ticker_strong cohort by definition; Class 4 catches 6 of 22 at default threshold). Mechanism class distinction: distinct from per-ticker price (A3) / per-sector (Spec 004/006) / prose-density (Spec 003) / LLM-extracted (Spec 007/008) / institutional flow (Spec X-1). Naming clarification: "Spec 012" / "Class 4" = Spec 008 design doc mechanism classification; NOT to be confused with C-4 institutional rotation = Spec X-1 (bear-side mechanism survey numbering). SC-010 default-on flip deferred until 30+ live shadow-mode fires accumulate (audit via `scripts/class4_macro_shadow_audit.py`). Per-spec retrospective: `claudedocs/spec-012-class-4-deployment-retrospective-2026-05-09.md` (PR #200).
