...
**Baseline** (post-2026-05-06 tech-debt sweep + TypedDict refactor — see `claudedocs/tech-debt-2026-05-05.md`): ruff = **0 errors** (down from 305 at scaffolding install — see commit `a0ec447`); mypy = **166 errors** (down from 175 after `TradingAgentsConfig` TypedDict in `default_config.py`). The TypedDict reduced `trading_graph.py` from 17 → 8 errors but did **not** materially reduce `conditional_logic.py` (40 errors) or the four LLM client modules (93 errors combined) — the actual dominant source there is langchain's `BaseMessage` union: `tool_calls` only exists on `AIMessage` but the message stream is typed as a 12-type union, generating one error per branch per call site. Fixing those cleanly would require either narrowing `isinstance(msg, AIMessage)` checks at each access or upstream langchain stubs; deferred. Net-new code must keep ruff at zero and should use the typed `get_config()` return for config access.
