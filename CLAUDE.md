...
**Test coverage** (as of 2026-05-08 post-WC-10 spec deployment): **1146 tests passing** (was 852 → 984 → 1022 → 1146 across 2026-05-06/07/08; +294 net since 2026-05-04 baseline), 81%+ project coverage. Anthropic + OpenAI clients ~95%, all 4 analyst factories + 5 debate-agent factories 100%, graph/setup.py 100%, conditional_logic.py 100%, trading_graph.py 88%, dataflows/interface.py 98%, memory.py 94% (per PR #116 corrected measurement; the 9.7% figure that appeared in 3 earlier coverage surveys was a `pytest-cov` collection-error artifact). New since 2026-05-04: Spec 003 contrarian gate (30 unit tests); Spec 002 within-ticker IC integration (8 tests); analyzer counterfactual-integration contract (2 tests); WC-10 (10 unit tests for `bin_scalar_to_tier` + 2 PM integration tests for filter-bypass mode). Spec 002 signal-lifecycle (Phases 0-2.5) at ~95%, Spec 001 bots-architecture (Phases 1-5) including Phase 4 BotLLMFactory live-validated, Spec 003 contrarian gate (Phases 1+2) live-validated end-to-end (SC-001 + SC-002), WC-10 v1 live-validated (`experiments/2026-05-08-001-wc-10-pilot/`). Routing-mismatch regression test (`tests/test_interface_routing.py::test_every_categorized_tool_has_impl_for_default_vendor`) added 2026-05-03 after experiment 008 caught the `get_insider_transactions × news_data × exa` mismatch. The 2026-05-08 mypy sweep (PRs #117-#129) added zero tests — typing-only changes; the existing 1146-test suite verified zero regressions across each PR.
