...
After 24 experiments + cross-experiment horizon sweep + per-ticker breakdown + Opus 4.7 model swap (NVDA + AAPL) + Opus 30-pair mixed basket + 3-period NVDA cross-validation (Q1 2026, Q4 2025, Q3 2025) + Phase D substrate exploration (XLK + multi-sector + XLE) + Phase C reasoning_evidence wiring + Spec 002 signal-lifecycle pipeline + Spec 001 bots-architecture (Phases 1-5) + Spec 003 analyst-stage contrarian gate (default-on @80%) + Spec 003.5 sector-baseline fallback + Spec 007 forward-catalyst gate (bull-active, bear-shadow) + Spec 008 Hybrid C calendar boost + Spec X-1 institutional rotation filter (default-shadow bear) + **SC-003 50-ticker signal-at-scale validation** + **Spec 008 SC-009 live A/B ablation (PRELIMINARY PASS-by-non-counterexample)** + **bear-side mechanism class survey CONCLUDES (6/6 evaluated; C-4 sole spec-eligible)** + **WC-10 v1 continuous-scalar pilot 2026-05-08 (n=20 paired, SC-007 ALT-A confirmed at 3.6× commit ratio — categorical bottleneck IS real, mode collapse is TWO-MECHANISM not unitary)** + **WC-10 v3 bear-regime test (Q4 2025 NVDA) RESOLVED 2026-05-08 evening: PARTIAL ALT-A — direction matches ALT-A (8/8 dates Buy/OW vs 5-tier 0 OW + 1 UW + 7 Hold) but α delta -0.22pp within ±100bps NULL region; Constitution VII v1.5.0 → v1.5.1 Patch D applied** + **WC-10 v2 expansion (n=100, in flight, ~8h remaining)** + **Spec 011 behavioral-additive operational procedure (codifies Constitution v1.4.6 invocation pattern)** + **Spec 009 WC-10 production deployment design surface complete** (5 of 7 spec-kit artifacts shipped — spec.md + plan.md + contracts/daily_signals_wc_10_flag.md + research.md + quickstart.md; tasks.md + MVP/tests/polish pending v2 verdict) + **BR-3 Squeak market-analyst structured-output pilot scaffolded** (sister hypothesis to WC-10 at analyst-stage; $8 T2 if launched per `experiments/2026-05-09-001-br3-squeak-market-analyst/`) + **v2 landing PR series fully pre-scaffolded** (4-PR landing playbook in `claudedocs/v2-landing-pr-series-bundle-template-2026-05-08.md` covers computation snippet + 6-cell verdict matrix + Spec 009 branch-selection workflow; ~75 min wall-clock when v2 lands), the architectural conclusion is captured in **`RESEARCH_FINDINGS.md`**. Forward roadmap in **`ROADMAP.md`**.
...
This is encoded in **Constitution Principle VII (Calibrated Abstention is a Valid Output)** added 2026-05-03 then re-amended after 006. **Amended 2026-05-08 to v1.5.0 with "Schema-induced abstention is NOT calibrated abstention" sub-section** after WC-10 v1 pilot empirically demonstrated that some Hold rates are categorical-schema artifacts (NVDA case: 8 of 10 5-tier Holds were schema-suppressed bullish reads that continuous-scalar mode emitted as Buy/OW with realized 21d α from +2.83% to +8.53%). **Further amended 2026-05-08 evening to v1.5.1 with "Bear-regime validation" paragraph** after WC-10 v3 PARTIAL ALT-A verdict on Q4 2025 NVDA cohort (α delta -0.22pp; direction matches ALT-A but magnitude < 1.0pp). VII still applies to commits whose evidence is GENUINELY BALANCED (the original framing); the v1.5.0 carve-out covers cases where evidence is one-directional but moderate-magnitude AND the schema lacks a partial-confidence tier; v1.5.1 documents the empirical magnitude bound (`|delta| < 1.0pp`) and notes that runtime monitoring via `scripts/wc_10_underperformance_monitor.py` (PR #146) suffices in lieu of regime-aware gating. Any new structural change that reduces Hold rate must justify in HYPOTHESIS.md which sub-population it targets — genuine ambiguity (NOT VII-eligible) or schema-induced collapse (VII-eligible per WC-10 precedent) — and document the directional asymmetry expectation.

## Read the constitution first

The design is governed by `.specify/memory/constitution.md`. Before non-trivial changes, read:

1. `.specify/memory/constitution.md` — **eight** core principles (Save Everything, One Experiment Per Change, Stay Cheap, No Production Claims, Steal Liberally, Spec Before Structural Change, **Calibrated Abstention is a Valid Output**, **Retrospective Before Spec for Backward-Looking Price Filters**). **Current version: v1.5.1** (2026-05-08 evening, PATCH — added "Bear-regime validation" paragraph to Principle VII sub-section per WC-10 v3 PARTIAL ALT-A verdict). Recent amendment history: v1.3.0 → v1.4.0 (forward-catalyst-class gate) → v1.4.1 (spec ships its retrospective) → v1.4.2 (magnitude fungibility) → v1.4.3 (additive-to-existing-filter gate) → v1.4.5 (Quality Gate #6 memory-log discipline) → v1.4.6 (behavioral-additive 4th interpretation; v1.4.4 draft ratified as v1.4.6 to preserve monotone numbering) → v1.5.0 (schema-induced abstention carve-out per WC-10 v1) → **v1.5.1 (bear-regime magnitude bound per WC-10 v3)**. The principles are constraints, not aspirations.
